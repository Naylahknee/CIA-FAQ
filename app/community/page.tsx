...
export default function CommunityPage() {
  ...
  async function createPost(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setNotice(""); const data = new FormData(event.currentTarget); data.set("body", composer); data.set("gifUrl", gifUrl); const response = await fetch("/api/community/posts", { method: "POST", body: data }); const result = await response.json(); if (!response.ok) setNotice(result.error); else { setComposer(""); setGifUrl(""); setMediaName(""); if (mediaRef.current) mediaRef.current.value=""; await loadFeed(); } setBusy(false); }
    ...
    <main className="community-shell auth-shell">
      ...
      <section className="auth-panel">
        <div className="auth-card">
          ...
          <a className="back-link" href="/">Back to the Guide &amp; FAQ</a>
        </div>
      </section>
    </main>
  );
  ...
  return <main className="community-shell"><header className="feed-header"><a className="community-brand" href="/" aria-label="Open the Guide and FAQ"><span className="brand-icon"><img src="/favicon.svg" alt="" /></span></a><nav><a href="/">Guide &amp; FAQ</a><a className="active" href="/community">Community</a></nav><div className="member-menu"><span className="avatar small">{initials(user.displayName)}</span><span>{user.displayName}</span><a href="/account">Account settings</a><button onClick={signout} aria-label="Sign out"><LogOut size={18}/></button></div></header>
    <div className="feed-layout"><aside className="feed-sidebar left-sidebar"><div className="member-card"><span className="avatar">{initials(user.displayName)}</span><strong>{user.displayName}</strong><small>Community member</small></div><nav><a className="active" href="/community"><MessageCircle size={19}/>Community feed</a><a href="/#walls-title"><ImageIcon size={19}/>Resource & memory walls</a><a href="/"><ChevronDown size={19}/>Guide &amp; FAQ</a></nav></aside>
