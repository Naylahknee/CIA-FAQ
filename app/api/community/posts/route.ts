...
export async function GET() {
  try {
    ...
    const posts = await getDb().select({ id: communityPosts.id, body: communityPosts.body, gifUrl: communityPosts.gifUrl, hasMedia: sql<boolean>`${communityPosts.mediaKey} is not null`, mediaType: communityPosts.mediaType, createdAt: communityPosts.createdAt, userId: communityUsers.id, displayName: communityUsers.displayName, role: communityUsers.role, isAnonymous: communityPosts.isAnonymous, topicName: communityTopics.name })
    ...
    const shaped = posts.map(post => ({ ...post, userId: post.isAnonymous && post.userId !== user.id && user.role !== "moderator" ? null : post.userId, displayName: post.isAnonymous ? "Anonymous member" : post.displayName, role: post.isAnonymous ? null : post.role, canDelete: post.userId === user.id || user.role === "moderator", imageUrl: post.hasMedia ? `/api/community/media/${post.id}` : null, comments: commentsResult.results.filter((row: any) => row.postId === post.id), reactions: reactionsResult.results.filter((row: any) => row.postId === post.id), myReaction: (mineResult.results.find((row: any) => row.postId === post.id) as any)?.reaction ?? null }));
