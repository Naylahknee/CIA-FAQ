...
export async function POST(request: Request) {
  try {
    ...
    if (!await isSafeImage(avatar, TYPES, MAX)) return noStoreJson({ error: "That file does not appear to be a valid image." }, { status: 415 });
    await ensureD1AuthSchema();
    ...
    await env.BUCKET.put(key, await avatar.arrayBuffer(), { httpMetadata: { contentType: avatar.type } });
